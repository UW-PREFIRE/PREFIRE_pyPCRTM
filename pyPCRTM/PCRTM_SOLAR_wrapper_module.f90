...
!
! This code is essentially a copy/paste of the original PCRTM_wrapper_module,
! updated to include those additional input user_types valid for PCRTM_SOLAR.
! Trying to make a generalized piece of code valid for both fortran libraries
! is a bit beyond my knowledge - I think I would need a runtime function that
! could tell if certain user_types or functions are defined (ones that are specific
! to PCRTM_SOLAR), inside the fortran. That is a bit beyond my expertise, so going
! with this (maybe unfortunate!) replication...
!

module pcrtm_solar_wrapper_module
  ...
  private TR_SOLUTION, EOF_SOLUTION, RT_SOLUTION, SOLAR_SOLUTION
  ...
  TYPE(PCRTM_SOLAR_LUT_DEF)   :: SOLAR_TAB(2)
  ...
  TYPE(PCRTM_SOLAR_DEF)                      :: SOLAR_SOLUTION
...
contains
  ...
  function pcrtmw_get_init_status() bind(c, name='pcrtmw_get_init_status')
    logical(c_bool) :: pcrtmw_get_init_status
    pcrtmw_get_init_status = INIT_STATUS
  end function pcrtmw_get_init_status

  function pcrtmw_get_run_status() bind(c, name='pcrtmw_get_run_status')
    logical(c_bool) :: pcrtmw_get_run_status
    pcrtmw_get_run_status = RUN_STATUS
  ...
  subroutine pcrtmw_init( sensor_id_in, &
    ...
    integer(c_int), intent(in), dimension(15) :: molindx_in
    real(c_float),  intent(in), dimension(15) :: scalfac_in
    ...
    ! get from env var
    call get_environment_variable('PCRTM_SOLAR_INPUT_DIR', INPUTDIR)
    ...
    PCRTM_STND%MOLINDX(1:15) = MOLINDX(1:15)
    PCRTM_STND%SCALFAC(1:15) = SCALFAC(1:15)
  ...
  subroutine pcrtmw_get_geom(geom) bind(c, name = 'pcrtmw_get_geom')
    real(c_float), intent(inout), dimension(6) :: geom
    ! line 120, PCRTM_ATMOSPHERE_LAYER.f90; note only the *'ed variables
    ...
    !     REAL(SINGLE) :: SOLAR_ZANG     * solar zenith in deg   
    !     REAL(SINGLE) :: SAT_AZIMUTH    * view azimuth in deg   
    !     REAL(SINGLE) :: SOLAR_AZIMUTH  * solar azimuth in deg   
    !  END TYPE PCRTM_GEOMETRY_TYPE

    geom(1) = GEOMETRY%PSFC
    geom(2) = GEOMETRY%POBS
    geom(3) = GEOMETRY%SATANG
    geom(4) = GEOMETRY%SAT_AZIMUTH
    geom(5) = GEOMETRY%SOLAR_ZANG
    geom(6) = GEOMETRY%SOLAR_AZIMUTH

  end subroutine pcrtmw_get_geom

  subroutine pcrtmw_set_geom(geom) bind(c, name = 'pcrtmw_set_geom')
    real(c_float), intent(in), dimension(6) :: geom
    GEOMETRY%PSFC = geom(1)
    GEOMETRY%POBS = geom(2)
    GEOMETRY%SATANG = geom(3)
    GEOMETRY%SAT_AZIMUTH = geom(4)
    GEOMETRY%SOLAR_ZANG = geom(5)
    GEOMETRY%SOLAR_AZIMUTH = geom(6)
  end subroutine pcrtmw_set_geom


  ! ATM structure: line 114 PCRTM_ATMOSPHERE_DEFINE.f90
  ...
  !    REAL(SINGLE), ALLOCATABLE  :: GASPROF(:,:)     ! GASPROF(NLAY,NMOL+1)
  !    REAL(SINGLE), ALLOCATABLE  :: AIRAMT(:)        ! AIRAMT(NLAY)
  !    REAL(SINGLE), ALLOCATABLE  :: DLAYDLEV(:)      ! DLAYDLEV(NLAY)
  !    INTEGER,      ALLOCATABLE  :: CLD_FLAG(:)      ! CLOUD FLAG (NLAY)
  !    REAL(SINGLE), ALLOCATABLE  :: PCLD(:)          ! CLOUD PARAM (NLAY)
  !    REAL(SINGLE), ALLOCATABLE  :: TAUCLD(:)        ! CLOUD tau (NLAY)
  !    REAL(SINGLE), ALLOCATABLE  :: DECLD(:)         ! CLOUD d_eff (NLAY)
  !    REAL(SINGLE)               :: TSKIN            ! skin temp
  !    REAL(SINGLE)               :: TSFC             ! surface temp (internal calc???)
  !    REAL(SINGLE), ALLOCATABLE  :: VSFC(:)          ! ??? (NMOL)
  !    REAL(SINGLE)               :: DTSFCDTLSFC,DTSFCDTLSFC_        ! ???
  !    REAL(SINGLE), ALLOCATABLE  :: DVSFCDVLSFC(:),DVSFCDVLSFC_(:)  ! ??? (NMOL)
  ...
  function pcrtmw_get_tskin() bind(c, name = 'pcrtmw_get_tskin')
    real(c_float) :: pcrtmw_get_tskin
    pcrtmw_get_tskin = ATM%TSKIN
  ...
  function pcrtmw_get_nlevels() bind(c, name = 'pcrtmw_get_nlevels')
    integer(c_int) :: pcrtmw_get_nlevels
    if (INIT_STATUS) then
       pcrtmw_get_nlevels = PCRTM_STND%NLEV
    end if
  end function pcrtmw_get_nlevels

  function pcrtmw_get_nlayers() bind(c, name = 'pcrtmw_get_nlayers')
    integer(c_int) :: pcrtmw_get_nlayers
    if (INIT_STATUS) then
       pcrtmw_get_nlayers = PCRTM_STND%NLAY
    end if
  end function pcrtmw_get_nlayers

  function pcrtmw_get_nchannels() bind(c, name = 'pcrtmw_get_nchannels')
    integer(c_int) :: pcrtmw_get_nchannels
    integer :: b
    pcrtmw_get_nchannels = 0
    if (INIT_STATUS .and. OUTPUT_CH_FLAG) then
       do b = 1, SIZE(EOF_SOLUTION)
          pcrtmw_get_nchannels = pcrtmw_get_nchannels + EOF_SOLUTION(b)%NCHBND
       end do
    end if
  end function pcrtmw_get_nchannels

  function pcrtmw_get_neof() bind(c, name = 'pcrtmw_get_neof')
    integer(c_int) :: pcrtmw_get_neof
    integer :: b
    pcrtmw_get_neof = 0
    if (INIT_STATUS) then
       do b = 1, SIZE(EOF_SOLUTION)
          pcrtmw_get_neof = pcrtmw_get_neof + EOF_SOLUTION(b)%NPCBND
       end do
    end if
  end function pcrtmw_get_neof

  function pcrtmw_get_nmonofreq() bind(c, name = 'pcrtmw_get_nmonofreq')
    integer(c_int) :: pcrtmw_get_nmonofreq
    integer :: b
    pcrtmw_get_nmonofreq = 0
    if (INIT_STATUS) then
       do b = 1, SIZE(EOF_SOLUTION)
          pcrtmw_get_nmonofreq = pcrtmw_get_nmonofreq + EOF_SOLUTION(b)%NREG
       end do
    end if
  end function pcrtmw_get_nmonofreq

  function pcrtmw_get_nbands() bind(c, name = 'pcrtmw_get_nbands')
    integer(c_int) :: pcrtmw_get_nbands
    pcrtmw_get_nbands = SIZE(EOF_SOLUTION)
...
end module pcrtm_solar_wrapper_module
