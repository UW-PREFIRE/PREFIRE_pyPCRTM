...
def perturb_loop():
    """
    run a perturbation loop - compute the forward run with each molecule
    species scaled to 10% of the standard atmosphere.
    """
    molindx = np.ones(15, dtype=np.int32)
    scalfac0 = np.ones(15, dtype=np.float32)

    F = setup_test_forward_bt(molindx, scalfac0)
    ...
    for n in range(15):
        scalfac = scalfac0.copy()
        scalfac[n] = 0.1
        F = setup_test_forward_bt(molindx, scalfac)
        datn = F.forward_rt()
        bts.append(datn['bt'])
...
def plot_perturbs(dat0, bta, fignum=22):
    """
    plot the output from perturb_loop()
    """
    mlist = ['H2O', 'CO2', 'O3', 'N2O', 'CO',
             'CH4', 'O2', 'NO', 'SO2', 'NO2',
             'NH3', 'NO3', 'C14', 'F11', 'F12']
    plot_scales = [1., 1., 1., 1., 1.,
                   1, 1, 100, 100, 1000,
                   100, 10, 10, 10, 10]
    ...
    for n in range(15):
        fig.clf()
        fig, (ax1, ax2) = plt.subplots(2,1,num=fignum)
        ax1.plot(dat0['wn'], dat0['bt'])
        ax1.plot(dat0['wn'], bta[:,n])
        ax2.plot(dat0['wn'], (bta[:,n] - dat0['bt']) * plot_scales[n])
        fig.suptitle('IASI Forward simulation with 0.1x '+mlist[n])
        ax1.set_ylabel('BT [K]')
        ax2.set_xlabel('wavenumber [1/cm]')
        if plot_scales[n] != 1:
            ax2.set_ylabel('Delta BT [K] x{:d}'.format(int(plot_scales[n])))
        else:
            ax2.set_ylabel('Delta BT [K]')
        fig.savefig('solar_forward_moltest_m{:02d}_{:s}.png'.format(n+1,mlist[n]))


def setup_test_forward_bt(molindx, scalfac):
    """
    create the PCRTM_SOLAR object, with a hardcoded temperature profile,
    and the input molindx/scalfac arrays.

    This is used by the function perturb_loop to look at the spectral
    "fingerprint" of each of the molecular species.
    """
